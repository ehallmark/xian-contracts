...
module_dir = join(dirname(dirname(dirname(abspath(__file__)))), 'xianmarket')

DAO_CONTRACT = 'con_xianmarket_dao_v1'
SPORTS_BETTING_CONTRACT = 'con_xianmarket_implementation_v1'
CURRENCY_CONTRACT = 'currency'
PHI_CONTRACT = 'con_xmt_xst001'
SPORTS_BETTING_ACTION = 'betting_market'
...
with open(join(dirname(module_dir), 'common', f'{CURRENCY_CONTRACT}.py'), 'r') as f:
    code = f.read()
    client.submit(code, name=CURRENCY_CONTRACT, signer='sys')
...
t1 = time.time()
with open(os.path.join(module_dir, f'{SPORTS_BETTING_CONTRACT}.py'), 'r') as f:
    code = f.read()
...
tau = get_currency_contract_for_signer('sys')
